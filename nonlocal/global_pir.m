...
% j4=find(lati>-65 & lati<-40);
i4=find(loni >= 70 & loni <= 73);
j4=find(lati >= -50 & lati <= -47);
y4=y(i4,j4);%南大洋-印度洋

% i5=find(loni>150 & loni<287);
% j5=find(lati>-65 & lati<-40);
i5=find(loni >= 167 & loni <= 170);
j5=find(lati >= -52 & lati <= -49);
y5=y(i5,j5);%南大洋-太平洋
% 
% i6=find(loni>287 & loni<360);
% j6=find(lati>-65 & lati<-40);
i6=find(loni >= 292 & loni <= 296);
j6=find(lati >= -54 & lati <= -50);
y6=y(i6,j6);%南大洋-大西洋

% i7=find(loni>0 & loni<25);
% j7=find(lati>-65 & lati<-40);
% y7=y(i7,j7);%南大洋-大西洋
%% pir大面图
figure
set(gca,'FontSize',15)
...
m_pcolor(x0,y0,nanconv(squeeze(pir_24ym(:,:,12)),h,'edge','nanout'));shading flat;
% m_pcolor(x0,y0,pis_year(:,:,40));shading flat;
c=colorbar('fontsize',15,'fontweight','bold');
% c.Location='southoutside';
% c.Position=[0.278902083333333,0.215044270833333,0.473075,0.034691620979946];
...
set(get(c,'Title'),'string','W/m^3','position',[71.56459062499994,110.2046155539997,0]);
clim([-5e-6 5e-6]); 
...
m_grid('linestyle','none','linewidth',1.2,'tickdir','out','xaxisloc',...
    'bottom','yaxisloc','left','fontsize',15,'BackgroundColor', [0.7 0.7 0.7]);
hold on;
...
exportgraphics(gcf,'C:\Users\1\Desktop\陈儒nonlocal论文\figure\jpg\pil_4°running_average.jpg','Resolution',300)
...
clim(clim_range);
% set(ha(2),'YTickLabel','none');
savefig('C:\Users\1\Desktop\陈儒nonlocal论文\figure\fig\pis_4°running_average.fig')
exportgraphics(gcf,'C:\Users\1\Desktop\陈儒nonlocal论文\figure\jpg\pis_4°running_average.jpg','Resolution',300)
...
exportgraphics(gcf,'C:\Users\1\Desktop\陈儒nonlocal论文\figure\jpg\pir_4°running_average.jpg','Resolution',300)
