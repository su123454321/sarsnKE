...
load('D:\nonlocal KE cascade Data\JR_sounew.mat')
M=288;
% M=96;

for k2=1:48
    [c1,lags1]=xcov(squeeze(JR_sgm(:,k2)),M-1,'coeff');
    for lag=1:M-1
...
raw_ylim = [-1.5 1]; 
y_range = raw_ylim(2) - raw_ylim(1);
final_ylim = [raw_ylim(1), raw_ylim(2) + y_range * 0.25];
ylim(final_ylim);

set(ax1, 'Box', 'off', 'XColor', 'k', 'YColor', 'k', 'TickDir', 'in', 'LineWidth', 1.0);
...
lgd = legend([p1],'\itJ^{g}_R','fontsize',15,'FontWeight','Bold','location','northeast');
set(lgd, 'Box', 'on');
drawnow;
...
target_deg = [7, 5, 3, 1];
tick_pos = 1 ./ (target_deg * aa);
ax2 = axes('Position', ax1.Position, 'XAxisLocation', 'top', 'YAxisLocation', 'right', 'Color', 'none', 'Box', 'off');
set(ax2, 'XLim', get(ax1, 'XLim'), 'YLim', get(ax1, 'YLim'), ...
    'XTick', tick_pos, 'XTickLabel', [], 'YTick', [], ...
    'XColor', 'k', 'YColor', 'k', 'TickDir', 'in', 'LineWidth', 1.0);

text_y_pos = final_ylim(2) - (final_ylim(2)-final_ylim(1)) * 0.01;
for t = 1:length(target_deg)
    text(ax1, tick_pos(t), text_y_pos, sprintf('%g^{\\circ}', target_deg(t)), ...
...
base_ylim = ylim; 
y_range = base_ylim(2) - base_ylim(1);
final_ylim = [base_ylim(1), base_ylim(2) + y_range * 0.25];
ylim(final_ylim);
...
linkaxes([ax1, ax2], 'x');
savefig('C:\Users\1\Desktop\陈儒nonlocal论文\figure\fig\kuroshio_jr.fig')
exportgraphics(gcf,'C:\Users\1\Desktop\陈儒nonlocal论文\figure\jpg\kuroshio_jr.jpg','Resolution',300)
...
savefig('C:\Users\1\Desktop\陈儒nonlocal论文\figure\fig\sou_jr_2010_2017.fig')
exportgraphics(gcf,'C:\Users\1\Desktop\陈儒nonlocal论文\figure\jpg\sou_jr_2010_2017.jpg','Resolution',300)